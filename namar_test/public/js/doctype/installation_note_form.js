...
(function () {
  ...
  if (!window.__namar_test_loaded_scripts["h"]) {
    window.__namar_test_loaded_scripts["h"] = true;
    // BEGIN legacy Client Script: h
    frappe.ui.form.on('Installation Note', {
        refresh: function(frm) {
            set_item_filter(frm);
            if (frm.doc.__islocal && frm.doc.custom_sales_order && !frm.doc.customer) {
                frappe.db.get_value('Sales Order', frm.doc.custom_sales_order, 'customer', function(v) {
                    if (v && v.customer) {
    ...
    function set_item_filter(frm) {
        if (frm.doc.custom_material_request) {
            frappe.call({
                method: 'make_installation_note_from_mr',
                args: {
                    action: 'get_qty',
