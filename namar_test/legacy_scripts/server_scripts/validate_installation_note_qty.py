...
if mr:
    ...
    for row in installed_data:
        installed_map[row.item_code] = frappe.utils.flt(row.total_installed)
    ...
    for item_code, qty in current_qty.items():
        ...
        already_installed = installed_map.get(item_code, 0)
        available = max_qty - already_installed
        if available < 0:
            available = 0
        ...
        if qty > available:
            errors.append(
                ...
                "مُركّب سابقاً: <b>" + str(frappe.utils.flt(already_installed, 2)) + "</b>)"
            )
