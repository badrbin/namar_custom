...
MODULE_PATH = ROOT / "namar_custom" / "mentions" / "reference_cleanup.py"
...
class ReferenceCleanupTests(unittest.TestCase):
    ...
    def test_patch_and_after_delete_hook_are_registered_without_cancel_hook(self):
        hooks = runpy.run_path(str(ROOT / "namar_custom" / "hooks.py"))["doc_events"]
        cleanup = "namar_custom.mentions.reference_cleanup.cleanup_deleted_reference"
        ...
        parser.read(ROOT / "namar_custom" / "patches.txt")
        self.assertIn("namar_custom.patches.v0_0_7.purge_deleted_reference_followups", parser["post_model_sync"])
