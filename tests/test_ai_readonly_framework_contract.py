...
from namar_custom.ai_readonly.print_resources import render_pdf
    ...
    def test_real_get_print_signature_accepts_scoped_document_arguments(self):
        for source in self.sources:
            with self.subTest(source=str(source)):
                ...
                with patch.dict(sys.modules, modules), patch('namar_custom.ai_readonly.print_resources.inline_images', side_effect=lambda content, *args: content):
                    for hidden in (0, 1):
