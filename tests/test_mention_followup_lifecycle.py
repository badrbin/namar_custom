...
EVENTS_PATH = ROOT / "namar_test" / "mentions" / "events.py"
MENTION_SERVICE_PATH = ROOT / "namar_test" / "mentions" / "service.py"
HOOKS_PATH = ROOT / "namar_test" / "hooks.py"
PATCHES_PATH = ROOT / "namar_test" / "patches.txt"
SYNC_PATCH_PATH = (
    ROOT
    / "namar_test"
    / "patches"
...
def run_sync_patch(events_module, fake_frappe):
    ...
    with patch.dict(
        sys.modules,
        {
            ...
            "namar_test.mentions.events": events_module,
...
class MentionFollowupLifecycleTestCase(unittest.TestCase):
    ...
    def test_patch_file_uses_explicit_pre_and_post_model_sections(self):
        ...
        self.assertIn(
            "namar_test.patches.v0_0_5.sync_converted_mention_followups",
