...
from namar_test.ai_readonly.policy import (DEFAULT_ROLE, METHODS, Denied, checked_filters,
    field_names, route, validate_policy, validate_request)
from namar_test.ai_readonly.print_resources import _fetch_image, inline_images, render_pdf, validate_html
...
def make_boundary(roles=None, args=None, path="/api/resource/Customer", verb="GET", settings_policy=None):
    ...
    sys.modules.pop("namar_test.ai_readonly.boundary", None)
    sys.modules.pop("namar_test.ai_readonly.builder", None)
    with patch.dict(sys.modules, {"frappe": fake}):
        boundary = importlib.import_module("namar_test.ai_readonly.boundary")
    sys.modules["namar_test.ai_readonly.boundary"] = boundary
...
class PolicyTests(unittest.TestCase):
    ...
    def test_internal_dispatch_is_not_public(self):
        with self.assertRaises(Denied):
            route("/api/method/namar_test.ai_readonly.boundary.dispatch", "GET", {}, METHODS)
...
class BoundaryTests(unittest.TestCase):
    ...
    def builder(self, frappe):
        with patch.dict(sys.modules, {"frappe": frappe}):
            return importlib.import_module("namar_test.ai_readonly.builder")
    ...
    def test_internal_dispatch_cannot_be_shadowed_by_script_or_override(self):
        ...
        with patch.dict(sys.modules, {utility.__name__: utility}):
            ...
            f.override_whitelisted_method = lambda method: method
            utility.get_server_script_map = lambda: {"_api": {"namar_test.ai_readonly.boundary.dispatch": "unsafe script"}}
            with self.assertRaises(Denied): b._actual_check_dispatch_route()
    ...
    def test_print_prunes_child_fields_before_and_after_controller(self):
        ...
        fmt = ModuleType("namar_test.ai_readonly.print_resources")
    ...
    def test_read_only_before_rollback_and_dispatch_arguments_cleared(self):
        ...
        b.enforce_request()
        self.assertEqual(calls, [("rollback", True), ("begin", {"read_only": True})])
        self.assertEqual(dict(f.form_dict), {"cmd": "namar_test.ai_readonly.boundary.dispatch"})
    ...
    def test_settings_schema_is_system_manager_only(self):
        ...
        schema = json.loads((root / "namar_test/namar_test/doctype/ai_read_only_settings/ai_read_only_settings.json").read_text())
