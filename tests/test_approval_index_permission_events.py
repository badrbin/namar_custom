...
PATH = Path(__file__).resolve().parents[1] / "namar_test/followups/approval_index_permission_events.py"
...
class ApprovalIndexPermissionEventTests(unittest.TestCase):
    def setUp(self):
        ...
        self.index = types.ModuleType("namar_test.followups.approval_index")
        ...
        modules = {"frappe": self.frappe, "namar_test.followups.approval_index": self.index}
        for name in (
