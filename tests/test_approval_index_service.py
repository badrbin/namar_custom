...
class IndexedServiceRuntime:
    def __init__(self, size=1):
        ...
        self.engine = ModuleType("namar_test.followups.approval_index")
    ...
    @contextmanager
    def installed(self):
        package = importlib.import_module("namar_test.followups")
        with (
            patch.dict(sys.modules, {"namar_test.followups.approval_index": self.engine}),
    ...
    def mentions_installed(self):
        service = ModuleType("namar_test.mentions.service")
        service.get_open_mention_count = Mock(return_value=7)
        package = ModuleType("namar_test.mentions")
        package.service = service
        with (
            patch.dict(sys.modules, {"namar_test.mentions": package, "namar_test.mentions.service": service}),
...
class ApprovalIndexServiceTestCase(unittest.TestCase):
    ...
    def test_import_failure_also_isolated_from_unified_count_and_list(self):
        ...
        def guarded_import(name, globals=None, locals=None, fromlist=(), level=0):
            if name == "namar_test.followups" and "approval_index" in fromlist:
